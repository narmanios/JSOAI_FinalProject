...
async function bubblechart() {
  ...
  const dataURL = "../data/output-module-one.json";

  fetch(dataURL)
    .then((response) => response.json())
    .then((data) => {
      data = data.filter(
        (entry) => entry.tweet_num >= 150 && entry.tweet_num <= 800
      );
      const sizeScale = d3
        .scaleSqrt()
        .domain([50, 500])
      ...
      d3.select("#keywordDropdown").on("change", function () {
        ...
        d3.select("#otherKeywordDropdown").property(
          "disabled",
          selectedKeyword !== "all"
        );
      });
      ...
      d3.select("#otherKeywordDropdown").on("change", function () {
        const selectedKeyword = d3.select(this).property("value").toLowerCase();
        updateBubbles(selectedKeyword);
        d3.select("#keywordDropdown").property(
          "disabled",
          selectedKeyword !== "all"
        );
      });
      ...
      function updateBubbles(selectedKeyword, dropdownBeingUsed) {
        svg
          ...
          .style("opacity", (d) =>
            selectedKeyword === "all" ||
            d.title.toLowerCase().includes(selectedKeyword)
              ? 1
              : 0.1
          );
        svg
        ...
        svg
          ...
          .on("mouseover", (event, d) => {
            if (
              ...
              tooltip
                .html(
                  `<span class="tooltip-key"><b>Story:</b></span> ${d.title}, <br><span class="tooltip-key"><b>Retweeted:</b></span> ${d.tweet_num}`
                )
      ...
      function renderBubbles(data) {
        ...
        const simulation = d3
          ...
          .force(
            ...
            d3
              .forceCollide()
              .radius((d) => sizeScale(d.tweet_num) + 2)
              .strength(0.5)
          )
          .on("tick", ticked);
